...
labels = ["18-24", "25-34", "35-44", "45-150"]
...
# 热力图：RFM × 兴趣渗透率
def plot_heatmap_pref(pvt, title, xlab, ylab):
...
#  交叉：生命周期 × 活跃度
cross_life_active = (
    df.groupby(["Lifecycle_Stage_Label","Active_Band"], observed=True)["User_ID"]
      .nunique()
      .unstack("Active_Band", fill_value=0)
      .astype(int)
)
# cross_life_active：行=生命周期 分层，列=活跃度档位，值=人数
# 用于识别：哪些分层已经从“成长期 → 成熟期”（高活跃占比高），
# 哪些分层高价值但活跃度下滑，需要提前干预。
...
# 生命周期 × 活跃度分档（人数热力图）
def plot_heatmap_life_active(pvt):
    if pvt is None or pvt.empty:
        return
    M = pvt.values
    plt.figure()
    im = plt.imshow(M, aspect="auto")
    plt.title("生命周期 × 活跃度分档（人数热力图）")
    plt.xlabel("活跃度分档")
    plt.ylabel("生命周期阶段")
    plt.xticks(range(M.shape[1]), [str(c) for c in pvt.columns], rotation=45)
    plt.yticks(range(M.shape[0]), list(pvt.index))
    plt.colorbar(im, fraction=0.046, pad=0.04)
    plt.tight_layout()
    plt.show()

plot_heatmap_life_active(cross_life_active)